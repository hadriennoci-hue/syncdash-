...
const createSchema = z.object({
  ...
  parentPostPk: z.number().int().positive().optional(),
  replyToExternalId: z.string().regex(/^\d+$/).optional(),
...
export async function POST(req: NextRequest) {
  ...
  const inserted = await db.insert(socialMediaPosts).values({
    ...
    parentPostPk: parsed.data.parentPostPk ?? null,
    replyToExternalId: parsed.data.replyToExternalId ?? null,
    publishedAt,
