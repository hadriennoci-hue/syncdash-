      ...
      {scanResults && (
        <div className="grid grid-cols-3 gap-3">
          {PLATFORMS.map((p) => {
            ...
            return (
              <div key={p} className={`border rounded p-3 text-xs ${isErr ? 'border-destructive' : 'border-green-500/40'}`}>
                ...
                {isErr
                  ...
                  : (() => {
                      ...
                      return (
                        <span>
                          <span className="text-green-600">{s.imported} new · {s.updated} updated</span>
              ...
              {filtered.length === 0
                ...
                : filtered.map((p: any) => (
                  <tr key={p.id} className="border-b border-border last:border-0 hover:bg-accent/50 transition-colors">
                    ...
                    <td className="px-3 py-2 max-w-[220px] truncate" title={p.title}>{p.title}</td>
                    <td className="px-3 py-2 text-center">
                      <PlatformBadge status={p.platforms.woocommerce?.status} />
                    </td>
                    <td className="px-3 py-2 text-center">
                      <PlatformBadge status={p.platforms.shopify_komputerzz?.status} />
                    </td>
                    <td className="px-3 py-2 text-center">
                      <PlatformBadge status={p.platforms.shopify_tiktok?.status} />
                    </td>
...
function PlatformBadge({ status }: { status: string | undefined }) {
  if (!status || status === 'missing')     return <span className="text-muted-foreground">—</span>
  if (status === 'synced')                 return <span className="text-green-600 font-medium">✓</span>
  if (status === 'differences')            return <span className="text-amber-500 font-medium">⚠</span>
